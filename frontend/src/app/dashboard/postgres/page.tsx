...
import { postgresTexts } from '@/constants/static/postgres/postgresTexts';
...
const Page = () => {
  ...
  useEffect(() => {
    ...
    const websocket = new WebSocket(
      `${backendUrl}/query/postgres-query/${accessToken}`
    );
    ...
    <React.Fragment>
      ...
      <div className="w-full flex my-6 flex-col">
        <div className="flex w-full justify-between items-center py-2 px-4">
          <div className=" font-semibold text-4xl text-Pri-Dark">Postgres</div>
          <div>
            <button
          ...
          <div className="w-1/2 flex flex-col gap-4  justify-between mb-12  p-6 bg-white rounded-2xl">
            <div className="overflow-y-scroll no-scrollbar h-full flex flex-col gap-2 text-justify bg-white">
              ...
              <div className=" w-full flex justify-center flex-col gap-2 h-full items-center">
                {history.length === 0 && (
                  <>
                    <SiPostgresql size={250} />
                    <div>Start conversation with our AI agent.</div>
                    <div className="flex gap-4  ">
                      {postgresTexts.map((text, index) => (
                        <>
                          <div
          ...
          <div className="flex flex-col w-1/2  border-2 p-6 mb-12 bg-white rounded-2xl">
            <div className="h-1/2 flex flex-col gap-6">
              ...
              <textarea
                ...
                name="comment"
                rows={4}
                className="block w-full rounded-md border-s py-1.5 text-gray-900  p-2 border border-Gray-Background outline-none placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-base sm:leading-6"
                defaultValue={''}
                value={rawQuery}
